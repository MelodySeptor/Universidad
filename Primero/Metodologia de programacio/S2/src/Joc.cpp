...
void tractarEvent(int tecla, FigTetris& fig, Fons& fons)
{
	...
	// utilitzar els mètodes de les classes FigTetris i Fons
		switch (tecla)
	{
	    case TECLA_ESQUERRA:{fig.moureFig(-1,0,fons); break;}
	    case TECLA_DRETA:{fig.moureFig(1,0,fons); break;}
	    case TECLA_ABAIX:{fig.moureFig(0,1,fons); break;}
	    case TECLA_ADALT:{fig.moureFig(0,-1,fons);break;}
	}
}
...
int joc(int nivell)
{
	...
	do
    {
        ...
        figura[indexFig].setPosX(( (INICI_X) + ( ( rand() % ( ( (FI_X - INICI_X - figura[indexFig].amplada()) / MIDA_Q ) + 1 ) ) * MIDA_Q )));
        ...
        fons.pintaFons();
        resultat.pintaResultats(nivell,punts);
