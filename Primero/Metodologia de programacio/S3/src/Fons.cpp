...
bool Fons::estaEncaixada(int col, int fila, int mascara[4][4], int extra)
//Retorna true si la posicio esta ocupada (?)
{
    ...
    for(int i=0; i<4; i++)
        ...
        for(int j=0; j<4; j++)
        {
            if(mascara[i][j] == 1)
            {
                if(fila == 0)
                {
                   if(m_tauler[fila+1][col+j] != COLOR_NEGRE && m_tauler[fila+1][col+j] != COLOR_GRIS)
                    {
                        return true;
                    }
                }
                if(m_tauler[fila-i+3+extra][col+j] != COLOR_NEGRE && m_tauler[fila-i+3+extra][col+j] != COLOR_GRIS)
                {
                    return true;
                }
            }
